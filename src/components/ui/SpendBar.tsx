  ...
  return (
    <div className="relative pt-1 pb-6">
      <div className="relative h-3 w-full overflow-hidden rounded-full bg-bg-2 ring-1 ring-inset ring-line">
        <div className="absolute inset-y-0 left-0 rounded-full bg-gradient-to-r from-allow/70 to-allow" style={{ width: `${Math.max(pct, spent > 0 ? 0.6 : 0)}%`, transition: 'width 1s var(--ease-out-expo)' }} />
      ...
      {tradePct > 0 && (
        <div className="absolute top-0 flex -translate-x-1/2 flex-col items-center" style={{ left: `${tradePct}%` }}>
          ...
          <span className="mt-0.5 whitespace-nowrap font-mono text-[9.5px] text-white">one max trade · {usd(perTrade, { compact: true })}</span>
        </div>
      )}
