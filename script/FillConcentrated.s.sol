...
/**
 * @notice Ships and fills a concentrated-liquidity position (XYCConcentrateSwap,
 * Uniswap-v3-style: quadratic invariant, virtual reserves, a real price range) gated
 * by MandateGate -- directly answers 1inch's own "sophisticated DeFi position"
 * criterion with a real on-chain fill, not just the local test that proved the math.
 * Range: 0.25x-4x around price 1 (our demo tokens mint 1:1) -- same fixed-point
 * convention swap-vm's own test builders use (sqrt(price * 1e18)).
 */
contract FillConcentrated is Script {
    ...
    function run() external {
        ...
        bytes memory program = bytes.concat(
            ...
            Salt.build(abi.encodePacked(uint256(5)))
        );
        ...
        vm.stopBroadcast();
        console2.log("Shipped concentrated-liquidity strategy, hash:");
        ...
        console2.log("MandateGate ALLOWED the concentrated-liquidity fill");
